...
def load_label_mappings():
    ...
    id2label = {
        int(k): v
        for k, v in labels_data["id2label"].items()
    }
...
class MultiLabelDataCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        labels = [
            torch.tensor(f["labels"], dtype=torch.float)
            for f in features
        ]
...
def main():
    ...
    encoded_dataset = preprocess_dataset(
        dataset,
        tokenizer
    )

    split = encoded_dataset["train"].train_test_split(
        test_size=0.1,
        seed=42
    )
    ...
    print("Saved combined loss curve -> loss_curve_combined.png")
    ...
    print("Saved training-only loss curve -> train_loss_only.png")
