...
# (Optional) counselchat if available locally
...
# Apply formatting
mentalchat = mentalchat.map(format_mentalchat).filter(lambda x: x is not None)
...
trainer = SFTTrainer(
    model=model,
    tokenizer=tokenizer,
    train_dataset=mentalchat, # Changed dataset to mentalchat
